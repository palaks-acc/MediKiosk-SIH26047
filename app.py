...
def speech_input(label, language_name, key_prefix, height=100):
    ...
    if not SPEECH_AVAILABLE:
        st.error(
            "Multilingual speech-to-text is unavailable because the "
            "`SpeechRecognition` Python package is not installed. "
            "Install it with: `pip install SpeechRecognition` and restart Streamlit."
        )
        return ""
...
# -----------------------------
    ...
    if SPEECH_AVAILABLE:
        st.success(
            f"🎙️ Multilingual speech-to-text ready — {selected_language} "
            f"({SPEECH_LANGUAGES.get(selected_language, 'en-IN')})"
        )
