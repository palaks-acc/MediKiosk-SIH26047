...
def detect_entities(text):
    ...
    medicines = []
    known_meds = [
        "metformin", "amlodipine", "paracetamol", "azithromycin",
        "omeprazole", "losartan", "atorvastatin", "insulin",
        "telmisartan", "levothyroxine"
    ]
    ...
    diagnoses = []
    known_dx = [
        "diabetes", "hypertension", "asthma", "hypothyroidism",
        "anemia", "migraine", "arthritis", "pneumonia"
    ]
    for dx in known_dx:
        if dx in t:
    ...
    investigations = []
    known_tests = [
        "hemoglobin", "hba1c", "blood pressure", "creatinine",
        "glucose", "cholesterol", "tsh", "ecg", "x-ray", "cbc"
    ]
    for test in known_tests:
        if test in t:
            investigations.append(test.upper() if test == "hba1c" else test.title())

    # Generic numeric lab extraction: e.g. HbA1c 7.2
    numeric_values = re.findall(
        r"\b(?:hba1c|hemoglobin|hb|glucose|creatinine|tsh)\s*[:=-]?\s*(\d+(?:\.\d+)?)",
        t,
    )
...
BASE_QUESTIONS = [
    ...
    ("personal_history", "Please describe relevant diet, sleep, activity, tobacco/alcohol use, or other lifestyle factors."),
]
...
def adaptive_questions(history):
    ...
    if any(x in complaint for x in ["pain", "ache"]):
        questions.insert(
            2, ("pain_site", "Where exactly is the pain, and does it move anywhere else?")
        )
        questions.insert(
            3, ("pain_character", "How would you describe it: pressure, burning, stabbing, throbbing, or something else?")
        )
        questions.insert(
            4, ("pain_triggers", "What makes it better or worse—movement, food, exertion, breathing, or rest?")
        )

    if any(x in complaint for x in ["fever", "temperature"]):
        questions.insert(
            2, ("fever_pattern", "What was the highest temperature you measured, and how did you measure it?")
        )

    if any(x in complaint for x in ["cough", "breath", "breathing"]):
        questions.insert(
            2, ("respiratory", "Do you have breathlessness, wheezing, chest discomfort, or coughing up blood?")
        )

    if any(x in complaint for x in ["stomach", "abdominal", "vomit", "diarr"]):
        questions.insert(
            2, ("gi", "Any vomiting, diarrhea, constipation, blood in stool, or change in appetite?")
        )
...
def make_fhir_bundle():
    ...
    questionnaire = {
        ...
        "item": [
            {"linkId": k, "text": k.replace("_", " ").title(),
    ...
    flag_observation = {
        ...
        "valueString": "; ".join(red) if red else "No configured red flags detected",
    }
...
with st.sidebar:
    ...
    st.divider()
    status = "🟢 Consent granted" if st.session_state.consent else "🟠 Consent pending"
    st.write(status)
...
if st.session_state.page == "Patient Intake":
    ...
    c1, c2 = st.columns(2)
    with c1:
        name = st.text_input("Patient name", value=st.session_state.patient.get("name", ""))
        age = st.number_input(
        ...
        sex = st.selectbox(
            ...
            index=["", "Male", "Female", "Other", "Prefer not to say"].index(
                st.session_state.patient.get("sex", "")
            )
        )
    with c2:
        abha = st.text_input(
        ...
        language = st.selectbox(
            "Preferred language",
            ["English", "Hindi", "Bengali", "Tamil", "Telugu", "Marathi", "Other"],
            index=0,
        )
    ...
    consent = st.checkbox(
        "I understand that my information will be used to prepare a structured clinical intake for physician review."
    )

    if st.button("Start Patient Intake", type="primary", disabled=not consent):
        ...
        if name:
            st.session_state.patient["local_id"] = save_patient(st.session_state.patient)
...
# -----------------------------
    ...
    st.caption(
        "The Streamlit demo uses text input for reliable offline demonstration. "
        "Audio capture can be added through st.audio_input and a speech-to-text service."
    )
...
# -----------------------------
    ...
    a["prakriti"] = st.selectbox(
        ...
        ["Not assessed", "Vata", "Pitta", "Kapha", "Vata-Pitta",
         "Pitta-Kapha", "Vata-Kapha", "Tridosha / mixed"],
        index=0,
    )
    a["vikriti"] = st.text_area("Vikriti (current imbalance)", value=a.get("vikriti", ""))
    a["agni"] = st.selectbox(
        "Agni (digestive capacity)",
        ["Not assessed", "Sama", "Vishama", "Tikshna", "Manda"],
    )
    a["koshtha"] = st.selectbox(
        "Koshtha (bowel nature)",
        ["Not assessed", "Mridu", "Madhyama", "Krura"],
    )
    ...
    a["dashavidha"] = st.text_area(
        ...
        help="Use for structured clinical documentation; do not treat this prototype as an autonomous Ayurvedic diagnostic system.",
    )
...
# -----------------------------
    ...
    files = st.file_uploader(
        "Upload documents",
        type=["pdf", "png", "jpg", "jpeg", "webp", "tiff", "bmp", "txt"],
        accept_multiple_files=True,
    )

    if files:
        for f in files:
            if f.name not in [x["filename"] for x in st.session_state.documents]:
                ...
                conn = sqlite3.connect(DB_PATH)
                conn.execute(
                    "INSERT INTO documents (patient_id, filename, extracted_text, uploaded_at) VALUES (?, ?, ?, ?)",
                    (
    ...
    if st.session_state.documents:
        ...
        st.session_state.timeline = build_timeline(st.session_state.documents)
...
# -----------------------------
    ...
    if not st.session_state.timeline:
        ...
        for item in st.session_state.timeline:
            st.markdown(f"### {item['date']} — {item['source']}")
            ...
            if item["values"]:
                st.warning("Numeric values detected: " + ", ".join(item["values"]))
            st.divider()
...
# -----------------------------
    ...
    with c1:
        if st.button("✅ Physician Accepts Draft", type="primary"):
            st.success("Draft marked as physician-reviewed for this demo session.")
    with c2:
...
# -----------------------------
    ...
    if st.session_state.fhir:
        ...
        fhir_json = json.dumps(st.session_state.fhir, indent=2, ensure_ascii=False)
        st.download_button(
            "Download FHIR JSON",
            data=fhir_json,
